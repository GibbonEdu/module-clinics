...
$URL = $_SESSION[$guid]['absoluteURL'].'/index.php?q=/modules/'.getModuleName($_POST['address']).'/identify.php&gibbonYearGroupID='.$gibbonYearGroupID.'&gibbonDepartmentID='.$gibbonDepartmentID;
        ...
        if (!$departmentalAccess) {
            ...
            if (empty($priorities) || empty($gibbonPersonIDs)) {
                ...
                foreach ($priorities as $priority) {
                    $innerFail = false ;
                    try {
                        $dataPriority = array('gibbonSchoolYearID' => $_SESSION[$guid]['gibbonSchoolYearID'], 'gibbonDepartmentID' => $gibbonDepartmentID, 'gibbonPersonID' => $gibbonPersonIDs[$count]);
                        $sqlPriority = 'SELECT *
                    ...
                    if (!$innerFail) {
                        if ($resultPriority->rowCount() == 1) { //Exists, so update
                            ...
                            try {
                                $data = array('priority' => $priority, 'gibbonSchoolYearID' => $_SESSION[$guid]['gibbonSchoolYearID'], 'gibbonDepartmentID' => $gibbonDepartmentID, 'gibbonPersonID' => $gibbonPersonIDs[$count]);
