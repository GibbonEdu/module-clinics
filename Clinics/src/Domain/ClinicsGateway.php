...
class ClinicsGateway extends QueryableGateway
{
    ...
    public function queryClinicsBySchoolYear(QueryCriteria $criteria, Int $gibbonSchoolYearID, String $gibbonYearGroupID = null, Bool $enforceLockEnrolment = false)
    {
        $query = $this
            ...
            ->cols(['clinicsClinicID', 'clinicsBlock.clinicsBlockID', 'clinicsBlock.sequenceNumber', 'clinicsBlock.name AS blockName', 'gibbonSchoolYear.name AS schoolYear', 'gibbonDepartment.name AS department', 'clinicsClinic.name', 'description', 'clinicsClinic.active', 'lockEnrolment', 'maxParticipants', 'gibbonSpace.name AS space'])
